...
[root@localhost ~]# kubectl top pod -n caihcloud | grep caihcloud
caihcloud-f954b5dcd-nd8mx            5m           108Mi

  caihcloud                   caihcloud-f954b5dcd-nd8mx                                260m (3%)     2250m (28%)  170Mi (2%)       1184Mi (18%)   16m
...
[root@localhost ~]# kubectl top pod -n caihcloud
NAME                                 CPU(cores)   MEMORY(bytes)
caihcloud-d8469d8f5-hs8ks            5m           125Mi

[root@localhost ~]# kubectl describe node | grep caihcloud
caihcloud                   caihcloud-d8469d8f5-hs8ks                                10m (0%)      2 (25%)     10Mi (0%)        1Gi (15%)      113s
...
# 总结
- kubectl top pod数据并不大,kata 开销很大
...
- systemd-cgtop只能查看到kata pod的mem数据，没有cpu数据？？
- 开启debug_console_enabled = true后，kata_overhead有65M mem开销，跑IO测试后，此开销会达上G
- 起一个kata pod空复杂mem开销达110.7M，跑IO测试后开销达上G
- 开启sandbox_cgroup_only=true时总的mem开销包括业务负载会限制在limit范围内（1G）